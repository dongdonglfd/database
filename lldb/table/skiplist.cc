...
auto SkipList<Key, Comparator>::RandomHeight() -> int {
  static const unsigned int kBranching = 4;//每一层有 1/4 的概率继续往上长
  int height = 1;
...
auto SkipList<Key, Comparator>::FindGreaterOrEqual(const Key &key,
                                                   SkipNode **prev) const
    -> SkipNode * {
  SkipNode *x = head_;cur = NewNode(key, height);
...
template class SkipList<unsigned long, lldb::Comparator>;
