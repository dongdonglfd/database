...
void Encoder::EncodeRecord(const VRecord &record) {
  ...
  uint32_t crc = lldb::crc32c::Value(header_ + 4, sizeof(header_) - 4);
  crc = lldb::crc32c::Extend(crc, compressed_buffer_.data(),
...
Status Decoder::DecodeHeader(Slice *src) {
  ...
  header_crc_ = lldb::crc32c::Value(src->data(), kHeaderSize - 4);
...
Status Decoder::DecodeRecord(Slice *src, VRecord *record, std::string *buffer) {
  ...
  uint32_t crc =
      lldb::crc32c::Extend(header_crc_, input.data(), input.size());
  if (crc != crc_) {
...
void VFileFooter::EncodeTo(std::string *dst) const {
  ...
  PutFixed32(dst, lldb::crc32c::Value(encoded.data(), encoded.size()));
}
...
Status VFileFooter::DecodeFrom(Slice *src) {
  ...
  if (!GetFixed32(src, &checksum) ||
      lldb::crc32c::Value(decoded.data(), decoded.size()) != checksum) {
